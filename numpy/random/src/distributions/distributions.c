...
static double standard_exponential_zig_unlikely(bitgen_t *bitgen_state, uint8_t idx,
                                                double x) {
  if (idx == 0) {
    return ziggurat_exp_r - log(next_double(bitgen_state));
...
static float standard_exponential_zig_unlikely_f(bitgen_t *bitgen_state,
                                                 uint8_t idx, float x) {
  if (idx == 0) {
    return ziggurat_exp_r_f - logf(next_float(bitgen_state));
...
static NPY_INLINE double next_gauss_zig(bitgen_t *bitgen_state) {
  ...
  for (;;) {
    ...
    if (idx == 0) {
      for (;;) {
        xx = -ziggurat_nor_inv_r * log(next_double(bitgen_state));
        yy = -log(next_double(bitgen_state));
        if (yy + yy > xx * xx)
...
float random_gauss_zig_f(bitgen_t *bitgen_state) {
  ...
  for (;;) {
    ...
    if (idx == 0) {
      for (;;) {
        xx = -ziggurat_nor_inv_r_f * logf(next_float(bitgen_state));
        yy = -logf(next_float(bitgen_state));
        if (yy + yy > xx * xx)
