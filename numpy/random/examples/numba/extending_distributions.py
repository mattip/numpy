...
def normals(n, bitgen):
    out = np.empty(n)
    for i in range(n):
        out[i] = random_gauss_zig(bitgen)
    return out
...
# Can also get address from x.ctypes.bitgen.value
bitgen_address = int(ffi.cast('uintptr_t', bitgen))

norm = normalsj(1000, bitgen_address)
