...
cdef class RandomState:
    ...
    *Compatibility Guarantee*
    ...
    def __reduce__(self):
        ...
        return __randomstate_ctor, (state['bitgen'],), state
    ...
    def get_state(self, legacy=True):
        ...
        if st['bitgen'] != 'MT19937' and legacy:
        ...
        if legacy:
            return (st['bitgen'], st['state']['key'], st['state']['pos'],
    ...
    def set_state(self, state):
        ...
        if isinstance(state, dict):
            if 'bitgen' not in state or 'state' not in state:
