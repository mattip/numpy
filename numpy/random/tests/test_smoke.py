...
class RNG(object):
    ...
    def setup_class(cls):
        ...
        cls.advance = None
        cls.seed = [12345]
        cls.rg = Generator(cls.bitgen(*cls.seed))
    ...
    def test_entropy_init(self):
        rg = Generator(self.bitgen())
        rg2 = Generator(self.bitgen())
        assert_(not comp_state(rg.bit_generator.state, rg2.bit_generator.state))

    def test_seed(self):
        rg = Generator(self.bitgen(*self.seed))
        rg2 = Generator(self.bitgen(*self.seed))
        rg.random_sample()
        rg2.random_sample()
    ...
    def test_uniform_float(self):
        rg = Generator(self.bitgen(12345))
        warmup(rg)
        state = rg.bit_generator.state
