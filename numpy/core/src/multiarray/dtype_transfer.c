...
hpy_get_decref_transfer_function(HPyContext *ctx, int aligned,
                            npy_intp src_stride,
                            HPy /* PyArray_Descr * */ h_src_dtype, 
                            PyArray_Descr *src_dtype_data,
        ...
        NpyAuxData *auxdata)
{
    ...
    if (data->decref_src.func != NULL) {
        ...
        if (data->decref_src.func(ctx, &data->decref_src.context,
...
/*
 * TODO HPY LABS PORT:
 * HACK ! this is binary compatible but should use signature of
 * 'get_loop_function'
 */
NPY_NO_EXPORT int
        ...
        NPY_ARRAYMETHOD_FLAGS *flags)
{
    ...
    *out_transferdata = PyMem_Malloc(sizeof(_any_to_object_auxdata));
    ...
    HPy arr = HPyArray_NewFromDescr_int(ctx,
            h_array_type, context->descriptors[0], 1,
            &shape, NULL, NULL,
            0, HPy_NULL, HPy_NULL, 0, 1);
    HPy_Close(ctx, h_array_type);
    data->arr = (PyArrayObject *)HPy_AsPyObject(ctx, arr);
    HPy_Close(ctx, arr);

    PyArray_Descr *descr_0 = PyArray_Descr_AsStruct(ctx, context->descriptors[0]);
    data->getitem = descr_0->f->getitem;
    HNPY_cast_info_init(ctx, &data->decref_src);

    if (move_references && PyDataType_REFCHK(descr_0)) {
        int needs_api;
        if (hpy_get_decref_transfer_function(ctx,
                aligned, strides[0], 
...
static int
hwrap_copy_swap_function(
        HPyContext *ctx,
        HPy /* (PyArray_Descr *) */ dtype, int should_swap,
        HPyArrayMethod_StridedLoop **out_stransfer,
        NpyAuxData **out_transferdata)
{
    // TODO HPY LABS PORT
...
static int
get_one_to_n_transfer_function(HPyContext *ctx, int aligned,
                            npy_intp src_stride, npy_intp dst_stride,
                            ...
                            int *out_needs_api)
{
    ...
    data->N = N;
    HNPY_cast_info_init(ctx, &data->decref_src);  /* In case of error */
 ...
 */
static int
get_n_to_n_transfer_function(HPyContext *ctx, int aligned,
                            npy_intp src_stride, npy_intp dst_stride,
                            HPy h_src_dtype, PyArray_Descr *src_dtype_data,
                            HPy h_dst_dtype, PyArray_Descr *dst_dtype_data,
                            int move_references,
                            ...
                            int *out_needs_api)
{
    ...
    if (N != 1) {
         ...
         */
        src_stride = src_dtype_data->elsize;
        dst_stride = dst_dtype_data != NULL ? dst_dtype_data->elsize : 0;
...
static int
get_subarray_broadcast_transfer_function(HPyContext *ctx, int aligned,
                            npy_intp src_stride, npy_intp dst_stride,
                            HPy h_src_dtype, PyArray_Descr *src_dtype_data,
                            HPy h_dst_dtype, PyArray_Descr *dst_dtype_data,
                            ...
                            int *out_needs_api)
{
    ...
    HNPY_cast_info_init(ctx, &data->decref_src);
    HNPY_cast_info_init(ctx, &data->decref_dst);
     ...
     */
    if (HPyArray_GetDTypeTransferFunction(ctx, aligned,
                    src_dtype_data->elsize, dst_dtype_data->elsize,
                    h_src_dtype, h_dst_dtype,
                    0,
                    &data->wrapped,
                    out_needs_api) != NPY_SUCCEED) {
    ...
    if (move_references && PyDataType_REFCHK(src_dtype_data)) {
        if (HPyArray_GetDTypeTransferFunction(ctx, aligned,
                        src_dtype_data->elsize, 0,
                        h_src_dtype, HPy_NULL,
                        1,
                        &data->decref_src,
                        out_needs_api) != NPY_SUCCEED) {
            NPY_AUXDATA_FREE((NpyAuxData *)data);
    ...
    if (PyDataType_REFCHK(dst_dtype_data)) {
        if (HPyArray_GetDTypeTransferFunction(ctx, aligned,
                        dst_dtype_data->elsize, 0,
                        h_dst_dtype, HPy_NULL,
                        1,
                        &data->decref_dst,
                        out_needs_api) != NPY_SUCCEED) {
            NPY_AUXDATA_FREE((NpyAuxData *)data);
    ...
    while (run--) {
        if (offsetruns[run].offset != -1) {
            offsetruns[run].offset *= src_dtype_data->elsize;
        }
    }
                            ...
                            int *out_needs_api)
{
    ...
    if (PyDataType_HASSUBARRAY(src_dtype_data)) {
        CAPI_WARN("subarray->shape is PyObject");
        if (!(PyArray_IntpConverter(src_dtype_data->subarray->shape,
                                            &src_shape))) {
        ...
        h_src_dtype = HPy_FromPyObject(ctx, (PyObject *)src_dtype_data->subarray->base);
        src_dtype_data = PyArray_Descr_AsStruct(ctx, h_src_dtype);
    }
    if (PyDataType_HASSUBARRAY(dst_dtype_data)) {
        CAPI_WARN("subarray->shape is PyObject");
        if (!(PyArray_IntpConverter(dst_dtype_data->subarray->shape,
                                            &dst_shape))) {
        ...
        h_dst_dtype = HPy_FromPyObject(ctx, (PyObject *)dst_dtype_data->subarray->base);
        dst_dtype_data = PyArray_Descr_AsStruct(ctx, h_dst_dtype);
}
    ...
    if ((dst_size == 1 && src_size == 1) || (
        ...
        return get_n_to_n_transfer_function(ctx, aligned,
                        src_stride, dst_stride,
                        h_src_dtype, src_dtype_data,
                        h_dst_dtype, dst_dtype_data,
                        move_references,
                        src_size,
    ...
    /* Copy the src value to all the dst values */
        ...
        return get_one_to_n_transfer_function(ctx, aligned,
                src_stride, dst_stride,
                h_src_dtype, src_dtype_data,
                h_dst_dtype, dst_dtype_data,
                move_references,
     ...
     */
        ...
        int ret = get_subarray_broadcast_transfer_function(ctx, aligned,
                        src_stride, dst_stride,
                        h_src_dtype, src_dtype_data,
                        h_dst_dtype, dst_dtype_data,
                        src_size, dst_size,
...
get_fields_transfer_function(HPyContext *ctx, int NPY_UNUSED(aligned),
                            ...
                            int *out_needs_api)
{
    HPy src_names = HPy_NULL, dst_names = HPy_NULL;
    HPy src_fld_dtype, dst_fld_dtype; // PyArray_Descr *
    npy_int i;
    size_t structsize;
    HPy_ssize_t field_count;
    ...
    if (!PyDataType_HASFIELDS(src_dtype_data)) {
        dst_names = HPyField_Load(ctx, h_dst_dtype, dst_dtype_data->names);
        field_count = HPy_Length(ctx, dst_names);
        ...
        data->field_count = 0;
        for (i = 0; i < field_count; ++i) {
            HPy key = HPy_GetItem_i(ctx, dst_names, i);
            HPy value = HPy_GetItem(ctx, fields_dst, key);
            HPy_Close(ctx, key);
            if (!HPy_ExtractDictItems_OiO(ctx, value, &dst_fld_dtype, &dst_offset, NULL)) {
                // error
                HPy_Close(ctx, value);
                PyMem_Free(data);
                res = NPY_FAIL;
         ...
         */
        if (move_references && PyDataType_REFCHK(src_dtype_data)) {
            if (hpy_get_decref_transfer_function(ctx, 0,
                                    src_stride,
                                    h_src_dtype, src_dtype_data,
    ...
    if (!PyDataType_HASFIELDS(dst_dtype_data)) {
        src_names = HPyField_Load(ctx, h_src_dtype, src_dtype_data->names);
        if (HPy_Length(ctx, src_names) != 1) {
            HPyErr_SetString(ctx, ctx->h_ValueError,
        ...
        HPy key = HPy_GetItem_i(ctx, src_names, i);
        HPy value = HPy_GetItem(ctx, fields_src, key);
        HPy_Close(ctx, key);
        if (!HPy_ExtractDictItems_OiO(ctx, value, &src_fld_dtype, &src_offset, NULL)) {
            // error
            HPy_Close(ctx, value);
            PyMem_Free(data);
            res = NPY_FAIL;
    ...
    src_names = HPyField_Load(ctx, h_src_dtype, src_dtype_data->names);
    if (HPy_Length(ctx, src_names) != field_count) {
        HPyErr_SetString(ctx, ctx->h_ValueError, "structures must have the same size");
        res = NPY_FAIL;
        goto finish;
    }
    ...
    for (i = 0; i < field_count; ++i) {
        ...
        value = HPy_GetItem(ctx, fields_src, key);
        HPy_Close(ctx, key);
        if (!HPy_ExtractDictItems_OiO(ctx, value, &src_fld_dtype, &src_offset, NULL)) {
            // error
            HPy_Close(ctx, dst_fld_dtype);
...
finish:
    ...
    HPy_Close(ctx, src_names);
    return res;
}

static int
get_decref_fields_transfer_function(HPyContext *ctx, int NPY_UNUSED(aligned),
                            npy_intp src_stride,
                            HPy h_src_dtype, PyArray_Descr *src_dtype_data,
                            HPyArrayMethod_StridedLoop **out_stransfer,
                            NpyAuxData **out_transferdata,
                            int *out_needs_api)
{
    ...
    names = HPyField_Load(ctx, h_src_dtype, src_dtype_data->names);
    field_count = HPy_Length(ctx, names);
    ...
    for (i = 0; i < field_count; ++i) {
        HPy key = HPy_GetItem_i(ctx, names, i);
        ...
        HPy_Close(ctx, value);
        PyArray_Descr *src_fld_dtype_data = PyArray_Descr_AsStruct(ctx, src_fld_dtype);
        if (PyDataType_REFCHK(src_fld_dtype_data)) {
            if (out_needs_api) {
                *out_needs_api = 1;
            }
            if (hpy_get_decref_transfer_function(ctx, 0,
...
static int
_hpy_dec_src_ref_nop(HPyContext *hctx,
...
hpy_get_decref_transfer_function(HPyContext *ctx, int aligned,
                            ...
                            int *out_needs_api)
{
    HNPY_cast_info_init(ctx, cast_info);

    /* If there are no references, it's a nop */
    if (!PyDataType_REFCHK(src_dtype_data)) {
        cast_info->func = &_hpy_dec_src_ref_nop;
        cast_info->auxdata = NULL;
        goto finalize;
    }
    /* If it's a single reference, it's one decref */
    else if (src_dtype_data->type_num == NPY_OBJECT) {
        if (out_needs_api) {
            *out_needs_api = 1;
        }
    ...
    else if (PyDataType_HASSUBARRAY(src_dtype_data)) {
    ...
    /* If there are fields, need to do each field */
    else if (PyDataType_HASFIELDS(src_dtype_data)) {
        if (out_needs_api) {
            *out_needs_api = 1;
        }

        if (get_decref_fields_transfer_function(ctx, aligned,
                            src_stride, h_src_dtype, src_dtype_data,
                            &cast_info->func, &cast_info->auxdata,
                            out_needs_api) < 0) {
            return NPY_FAIL;
        }
        ...
        //         "unsupported DType '%S'.", h_src_dtype);
    ...
    cast_info->descriptors[0] = HPy_Dup(ctx, h_src_dtype);
...
h_init_cast_info(HPyContext *ctx,
        HNPY_cast_info *cast_info, NPY_CASTING *casting, npy_intp *view_offset,
        HPy src_dtype, HPy dst_dtype, int main_step)
{
    //PyArray_Descr *src_dtype_data;
    //PyArray_Descr *dst_dtype_data;
    HPy src_dtype_meta = HNPY_DTYPE(ctx, src_dtype);
    HPy dst_dtype_meta = HNPY_DTYPE(ctx, dst_dtype);
    HPy cast_info_descr_0 = HPy_NULL;
    HPy cast_info_descr_1 = HPy_NULL;
    PyArray_DTypeMeta *src_dtype_meta_data = PyArray_DTypeMeta_AsStruct(ctx, src_dtype_meta);
    PyArray_DTypeMeta *dst_dtype_meta_data = PyArray_DTypeMeta_AsStruct(ctx, dst_dtype_meta);
    HPy meth = HPyArray_GetCastingImpl(ctx, src_dtype_meta, src_dtype_meta_data, dst_dtype_meta, dst_dtype_meta_data);
    if (HPy_IsNull(meth)) {
        goto fail;
    }
    ...
    cast_info->context.caller = HPy_NULL;
    cast_info->context.method = meth;

//    CAPI_WARN("h_init_cast_info: uses legacy NPY_cast_info");
//    PyArray_Descr *in_descr[2] = {
//            (PyArray_Descr *)HPy_AsPyObject(ctx, src_dtype),
//            (PyArray_Descr *)HPy_AsPyObject(ctx, dst_dtype)};
    HPy in_descr[2] = {src_dtype, dst_dtype};

//    PyArray_DTypeMeta *dtypes[2] = {
//            (PyArray_DTypeMeta *)HPy_AsPyObject(ctx, src_dtype_meta),
//            (PyArray_DTypeMeta *)HPy_AsPyObject(ctx, dst_dtype_meta)};
    HPy dtypes[2] = {src_dtype_meta, dst_dtype_meta};

    PyArrayMethodObject *meth_data = PyArrayMethodObject_AsStruct(ctx, meth);
    *casting = meth_data->resolve_descriptors(ctx,
            cast_info->context.method, dtypes,
            in_descr, cast_info->descriptors, view_offset);

//    Py_DECREF(in_descr[0]);
//    Py_DECREF(in_descr[1]);
//    Py_DECREF(dtypes[0]);
//    Py_DECREF(dtypes[1]);
...
//    cast_info_descr_0 = HPy_FromPyObject(ctx, (PyObject *)cast_info->descriptors[0]);
//    cast_info_descr_1 = HPy_FromPyObject(ctx, (PyObject *)cast_info->descriptors[1]);
//
//    assert(HPyArray_DescrCheck(ctx, cast_info_descr_0));
//    assert(HPyArray_DescrCheck(ctx, cast_info_descr_1));
//
//    if (!main_step && NPY_UNLIKELY(
//            !HPy_Is(ctx, src_dtype, cast_info_descr_0) ||
//            !HPy_Is(ctx, dst_dtype, cast_info_descr_1))) {
    assert(HPyArray_DescrCheck(ctx, cast_info->descriptors[0]));
    assert(HPyArray_DescrCheck(ctx, cast_info->descriptors[1]));
    if (!main_step && NPY_UNLIKELY(
            !HPy_Is(ctx, src_dtype, cast_info->descriptors[0]) ||
            !HPy_Is(ctx, dst_dtype, cast_info->descriptors[1]))) {
        /*
    ...
    HPy_Close(ctx, src_dtype_meta);
    HPy_Close(ctx, dst_dtype_meta);
    return 0;

fail:
    HPy_Close(ctx, src_dtype_meta);
    HPy_Close(ctx, dst_dtype_meta);
    HPy_Close(ctx, cast_info_descr_0);
    HPy_Close(ctx, cast_info_descr_1);
    return -1;
}


static void
_hpy_clear_cast_info_after_get_loop_failure(HPyContext *ctx, HNPY_cast_info *cast_info)
{
...
{
    _hpy_clear_cast_info_after_get_loop_failure(npy_get_context(), cast_info);
}
 ...
 */
static int
h_define_cast_for_descrs(HPyContext *ctx,
        int aligned,
        npy_intp src_stride, npy_intp dst_stride,
        HPy src_dtype, HPy dst_dtype,
        int move_references,
        HNPY_cast_info *cast_info, int *out_needs_api)
{
    ...
    int must_wrap = (!aligned &&
        (PyArrayMethodObject_AsStruct(ctx, cast_info->context.method)->flags & NPY_METH_SUPPORTS_UNALIGNED) == 0);

    /*
     * Wrap the input with an additional cast if necessary.
     */
    if (NPY_UNLIKELY(!HPy_Is(ctx, src_dtype, cast_info->descriptors[0]) || must_wrap)) {
        ...
        if (from_view_offset == 0 && !must_wrap) {
            ...
            castdata.from.func = &_hpy_dec_src_ref_nop;  /* avoid NULL */
            NPY_cast_info_xfree(&castdata.from);
        }
        else {
            /* Fetch the cast function and set up */
            HPyArrayMethod_Context *context = &castdata.from.context;
            PyArray_Descr *descriptors_0 = PyArray_Descr_AsStruct(ctx, cast_info->descriptors[0]);
            npy_intp strides[2] = {src_stride, descriptors_0->elsize};
            NPY_ARRAYMETHOD_FLAGS flags;
     ...
     */
    if (NPY_UNLIKELY(!HPy_Is(ctx, dst_dtype, cast_info->descriptors[1]) || must_wrap)) {
        ...
        if (to_view_offset == 0 && !must_wrap) {
            /* This step is not necessary and can be skipped. */
            castdata.to.func = &_hpy_dec_src_ref_nop;  /* avoid NULL */
            HNPY_cast_info_xfree(ctx, &castdata.to);
        }
        else {
            /* Fetch the cast function and set up */
            HPyArrayMethod_Context *context = &castdata.to.context;
            PyArray_Descr *descriptors_1 = PyArray_Descr_AsStruct(ctx, cast_info->descriptors[1]);
            npy_intp strides[2] = {descriptors_1->elsize, dst_stride};
            NPY_ARRAYMETHOD_FLAGS flags;
            PyArrayMethodObject *method = PyArrayMethodObject_AsStruct(ctx, context->method);
            if (method->get_strided_loop(ctx,
    ...
    HPyArrayMethod_Context *context = &cast_info->context;
    npy_intp strides[2] = {src_stride, dst_stride};
    NPY_ARRAYMETHOD_FLAGS flags;
    if (PyArrayMethodObject_AsStruct(ctx, context->method)->get_strided_loop(ctx,
            context, aligned, move_references, strides,
            &cast_info->func, &cast_info->auxdata, &flags) < 0) {
        _hpy_clear_cast_info_after_get_loop_failure(ctx, cast_info);
        goto fail;
    }
  ...
  fail:
    HNPY_cast_info_xfree(ctx, &castdata.main);
    HNPY_cast_info_xfree(ctx, &castdata.from);
    HNPY_cast_info_xfree(ctx, &castdata.to);
    return -1;
}
                            ...
                            int *out_needs_api)
{
    ...
    int res = HPyArray_GetDTypeTransferFunction(ctx, aligned, src_stride,
            dst_stride, h_src_dtype, h_dst_dtype, move_references,
                            ...
                            int *out_needs_api)
{
    ...
    if (HPy_IsNull(dst_dtype)) {
        ...
        return hpy_get_decref_transfer_function(ctx, aligned,
                                ...
                                src_dtype, PyArray_Descr_AsStruct(ctx, src_dtype),
                                cast_info,
                                out_needs_api);
    }

    int res = h_define_cast_for_descrs(ctx, aligned,
            src_stride, dst_stride,
...
hwrap_aligned_transferfunction(
        ...
        HPy /* (PyArray_Descr *) */ src_dtype,
        HPy /* (PyArray_Descr *) */ dst_dtype,
        HPy /* (PyArray_Descr *) */ src_wrapped_dtype,
        HPy /* (PyArray_Descr *) */ dst_wrapped_dtype,
        HPyArrayMethod_StridedLoop **out_stransfer,
        NpyAuxData **out_transferdata, int *out_needs_api)
{
    must_wrap = must_wrap | !aligned;
        ...
        int *out_needs_api, int allow_wrapped)
{
    ...
    if (src_dtype_data->type_num == dst_dtype_data->type_num) {
        /*
         ...
         */
        return hwrap_copy_swap_function(ctx, src_dtype,
                PyDataType_ISNOTSWAPPED(src_dtype_data) !=
                PyDataType_ISNOTSWAPPED(dst_dtype_data),
                out_stransfer, out_transferdata);
    }

    if (hget_legacy_dtype_cast_function(
            ctx,
            aligned,
            src_stride, dst_stride,
            src_dtype, dst_dtype,
     ...
     */
    HPy src_wrapped_dtype = hensure_dtype_nbo(ctx, src_dtype);
    if (HPy_IsNull(src_wrapped_dtype)) {
        goto fail;
    }
    HPy dst_wrapped_dtype = hensure_dtype_nbo(ctx, dst_dtype);
    if (HPy_IsNull(dst_wrapped_dtype)) {
        goto fail;
    }
    int res = hwrap_aligned_transferfunction(ctx,
                      ...
                      int move_references)
{
    ...
    char *args[2] = {src, dst};
    npy_intp strides[2] = {src_stride, dst_stride};
    cast_info.func(npy_get_context(), &cast_info.context, args, &count, strides, cast_info.auxdata);
