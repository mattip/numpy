        ...
        int *out_needs_api, int *out_needs_wrap)
{
    ...
    *out_needs_wrap = !aligned ||
    ...
    PyArray_Descr *py_src_dtype = HPy_AsPyObject(ctx, src_dtype);
    ...
    if (PyArray_ISNBO(src_dtype_data->byteorder)) {
        ...
        tmp_dtype = HPy_FromPyObject(ctx, py_tmp_dtype);
    ...
    data->aip = HPy_AsPyObject(ctx, h_aip);
    HPy_Close(ctx, h_aip);
    /*
     * TODO: This is a hack so the cast functions have an array.
     *       The cast functions shouldn't need that.  Also, since we
    ...
    if (PyArray_ISNBO(dst_dtype_data->byteorder)) {
        ...
        PyArray_Descr *py_dst_dtype = HPy_AsPyObject(ctx, dst_dtype);
    ...
    data->aop = HPy_AsPyObject(ctx, h_aop);
        ...
        HPy src_dtype, HPy dst_dtype, int main_step)
{
    ...
    HPy meth = HPyArray_GetCastingImpl(ctx, src_dtype_meta, dst_dtype_meta);
    if (HPy_IsNull(meth)) {
        goto fail;
    }
        ...
        HNPY_cast_info *cast_info, int *out_needs_api)
{
    ...
    if (NPY_UNLIKELY(!HPy_Is(ctx, src_dtype, cast_info->descriptors[0]) || must_wrap)) {
        // TODO HPY LABS PORT: cut off
        hpy_abort_not_implemented("h_define_cast_for_descrs: branch 0");
...
//        }
    ...
    if (NPY_UNLIKELY(!HPy_Is(ctx, dst_dtype, cast_info->descriptors[1]) || must_wrap)) {
        // TODO HPY LABS PORT: cut off
        hpy_abort_not_implemented("h_define_cast_for_descrs: branch 1");
...
//        }
    ...
    if (PyArrayMethodObject_AsStruct(ctx, context->method)->get_strided_loop(ctx,
        ...
        _hclear_cast_info_after_get_loop_failure(ctx, cast_info);
        goto fail;
    }
