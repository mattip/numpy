...
PyArray_Max(PyArrayObject *ap, int axis, PyArrayObject *out)
{
    ...
    ret = PyArray_GenericReduceFunction(arr, N_OPS_GET(maximum), axis,
...
PyArray_Min(PyArrayObject *ap, int axis, PyArrayObject *out)
{
    ...
    ret = PyArray_GenericReduceFunction(arr, N_OPS_GET(minimum), axis,
                                        PyArray_DESCR(arr)->type_num, out);
    Py_DECREF(arr);
    return ret;
}
...
PyArray_Ptp(PyArrayObject *ap, int axis, PyArrayObject *out)
{
    ...
    Py_DECREF(arr);
    if (out) {
        ret = PyObject_CallFunction(N_OPS_GET(subtract), "OOO", out, obj2, out);
    }
    else {
                  ...
                  int variance, int num)
{
    ...
    obj1 = PyArray_GenericReduceFunction((PyArrayObject *)obj3, N_OPS_GET(add),
                                         axis, rtype, NULL);
    Py_DECREF(obj3);
    Py_DECREF(arr2);
    if (obj1 == NULL) {
    ...
    if (!variance) {
        ...
        /* sqrt() */
        ret = PyArray_GenericUnaryFunction(arr1, N_OPS_GET(sqrt));
        Py_DECREF(arr1);
    }
    if (ret == NULL) {
        return NULL;
...
PyArray_Sum(PyArrayObject *self, int axis, int rtype, PyArrayObject *out)
{
    ...
    ret = PyArray_GenericReduceFunction((PyArrayObject *)arr, N_OPS_GET(add), axis,
                                        rtype, out);
    Py_DECREF(arr);
    return ret;
}
...
PyArray_Round(PyArrayObject *a, int decimals, PyArrayObject *out)
{
    ...
    if (decimals >= 0) {
        ...
        if (decimals == 0) {
            if (out) {
                return PyObject_CallFunction(N_OPS_GET(rint), "OO", a, out);
            }
            return PyObject_CallFunction(N_OPS_GET(rint), "O", a);
        }
        op1 = N_OPS_GET(multiply);
        op2 = N_OPS_GET(true_divide);
    }
    else {
        op1 = N_OPS_GET(true_divide);
        op2 = N_OPS_GET(multiply);
        decimals = -decimals;
    }
    if (!out) {
        if (PyArray_ISINTEGER(a)) {
    ...
    tmp = PyObject_CallFunction(N_OPS_GET(rint), "OO", ret, ret);
    if (tmp == NULL) {
        Py_DECREF(ret);
        ret = NULL;
        goto finish;
...
PyArray_Mean(PyArrayObject *self, int axis, int rtype, PyArrayObject *out)
{
    ...
    obj1 = PyArray_GenericReduceFunction(arr, N_OPS_GET(add), axis,
                                         rtype, out);
    ...
    if (!out) {
        ...
        ret = PyObject_CallFunction(N_OPS_GET(divide), "OOO", out, obj2, out);
    }
    Py_DECREF(obj1);
    Py_DECREF(obj2);
    return ret;
...
_slow_array_clip(PyArrayObject *self, PyObject *min, PyObject *max, PyArrayObject *out)
{
    ...
    if (max != NULL) {
        res1 = _GenericBinaryOutFunction(self, max, out, N_OPS_GET(minimum));
        if (res1 == NULL) {
            return NULL;
        }
    }
...
PyArray_Clip(PyArrayObject *self, PyObject *min, PyObject *max, PyArrayObject *out)
{
    ...
    if (func == NULL) {
        if (min == NULL) {
            return PyObject_CallFunctionObjArgs(N_OPS_GET(minimum), self, max, out, NULL);
        }
        else if (max == NULL) {
            return PyObject_CallFunctionObjArgs(N_OPS_GET(maximum), self, min, out, NULL);
        }
        else {
            return PyObject_CallFunctionObjArgs(N_OPS_GET(clip), self, min, max, out, NULL);
        }
    }
              ...
              int rtype, PyArrayObject *out)
{
    ...
    ret = PyArray_GenericReduceFunction((PyArrayObject *)diag, N_OPS_GET(add), -1, rtype, out);
    Py_DECREF(diag);
    return ret;
}
