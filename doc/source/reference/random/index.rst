...
number of different BitGenerators, and exposes many different probability
...
routines. The legacy `mtrand.RandomState` random number routines are still
available, but limited to a single generator.
...
which will be faster than the legacy methods in `mtrand.RandomState`
